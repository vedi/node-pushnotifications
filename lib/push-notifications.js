...
PushNotifications.prototype.send = function (pushId, data, callback) {
  ...
  if (regIdsAPN[0] != undefined) {
    ...
    messageAPN.badge = data.data.badge || _this.settings.apn.badge;
    messageAPN.sound = data.data.sound || _this.settings.apn.defaultData.sound;
    messageAPN.alert = data.data.title;
    messageAPN.category = data.data.category;
