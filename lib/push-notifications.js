...
PushNotifications.prototype.send = function (pushId, data, callback) {
  ...
  if (regIdsGCM[0] != undefined) {
    if (!gsmData) {
      ...
      _.extend(gsmData, _this.settings.gcm.dataDefaults);

      if (data.priority) {
        gsmData.priority = data.priority;
        delete gsmData.data.priority;
      }
    }

    messageGCM = new gcm.Message(gsmData);
  ...
  if (regIdsAPN[0] != undefined) {
    ...
    messageAPN.badge = data.badge || _this.settings.apn.badge;
    messageAPN.sound = data.sound || _this.settings.apn.defaultData.sound;
    messageAPN.alert = data.title;
    messageAPN.category = data.category;
    if (data.priority) {
      messageAPN.priority = data.priority;
